...
async function seedData() {
  ...
  try {
    ...
    for (const province of drcProvinces) {
      await db.insert(provinces).values(province).onDuplicateKeyUpdate({ set: { name: province.name } });
    }
    console.log(`✅ Inserted ${drcProvinces.length} provinces`);

    // Insert sample addresses
    console.log("🏠 Inserting sample addresses...");
    for (const address of sampleAddresses) {
      await db.insert(addresses).values(address).onDuplicateKeyUpdate({ set: { fullAddress: address.fullAddress } });
    }
    console.log(`✅ Inserted ${sampleAddresses.length} sample addresses`);
